...
namespace agon::optim {
    ...
    void SGD<Ts...>::load_from_bin(const std::string& path_str) {
        ...
        if (!in_file) throw std::runtime_error("Failed to open file for reading: " + path_str);
        ...
        std::apply([&](auto&... params) {
            (std::ranges::for_each(params.begin(), params.end(), [&](auto& param) {
                ...
                in_file.read(reinterpret_cast<char*>(param.data_ptr()), param.size() * sizeof(T));
                in_file.read(reinterpret_cast<char*>(mom.data()), param.size() * sizeof(T));
            }), ...);
        }, this->parameters_.data);
    }
    ...
    void SGD<Ts...>::save_to_bin(const std::string& path_str) const {
        ...
        std::apply([&](auto&... params) {
            (std::ranges::for_each(params.begin(), params.end(), [&](auto& param) {
                ...
                out_file.write(reinterpret_cast<const char*>(param.data_ptr()), param.size() * sizeof(T));
                out_file.write(reinterpret_cast<const char*>(mom.data()), param.size() * sizeof(T));
            }), ...);
        }, this->parameters_.data);
    }
}
