...
struct SVRGState : public OptimizerState {
  detail::ExtractedVector<TypeTuple> ref_exact{};
  detail::ExtractedVector<TypeTuple> ref_est{};
  detail::ExtractedVector<TypeTuple> ref_data{};
...
class SVRG : public Optimizer<DedupedPack> {
  ...
  template <typename T>
  detail::ExtractedVector<T>& ref_exact() {
    return std::get<detail::ExtractType_t<T>>(state_.ref_exact);
  }
  template <typename T>
  detail::ExtractedVector<T>& ref_est() {
    return std::get<detail::ExtractType_t<T>>(state_.ref_est);
  }
  template <typename T>
  detail::ExtractedVector<T>& ref_data() {
    return std::get<detail::ExtractType_t<T>>(state_.ref_data);
  }
